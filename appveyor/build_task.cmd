...
setlocal enableextensions enabledelayedexpansion
	...
	for %%z in (%ZTS_STATES%) do (
        ...
        echo Copying built files into build/asset dir C:\obj\Release_TS\php-8.*.*-dev-Win32-vc15-x86.zip
        rem xcopy "%PHP_BUILD_OBJ_DIR%\Release_TS\php-8.*.*-dev-Win32-vc15-x86.zip" "%APPVEYOR_BUILD_FOLDER%\build\" /s /i /Y
        powershell -NoP -NonI -Command "Expand-Archive -Force -Path '%PHP_BUILD_OBJ_DIR%\Release_TS\php-8.*.*-dev-Win32-vc15-x86.zip' -DestinationPath '%APPVEYOR_BUILD_FOLDER%\build\'"
